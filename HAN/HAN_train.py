...
tf.app.flags.DEFINE_string("ckpt_dir", "G:/GitHubLabeling/HAN_checkpoint/", "checkpoint location for the model")
...
tf.app.flags.DEFINE_integer("num_sentences", 8, "number of sentences in the document")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        han = HierarchicalAttention(FLAGS.num_classes, FLAGS.learning_rate, FLAGS.batch_size,
                                    FLAGS.decay_steps, FLAGS.decay_rate, FLAGS.sequence_length, FLAGS.num_sentences,
                                    FLAGS.embed_size, FLAGS.hidden_size, FLAGS.is_training, FLAGS.word2vec_model_path,
        ...
        summary_writer = tf.summary.FileWriter('HAN_with_summaries', sess.graph)
        ...
        for epoch in range(curr_epoch, FLAGS.num_epochs):
            ...
            while True:
                ...
                x, y = next_batch(input_data[start:end], batch_size, FLAGS.num_classes, FLAGS.sequence_length,
                                  han.w2vModel, FLAGS.embed_size)
                if x is None:
                    break
                feed_dict = {han.input_x: x,
                             han.input_y: y,
                             han.dropout_keep_prob: 0.5}
                curr_loss, curr_prec, curr_reca, _ = sess.run(
                    [han.loss_val, han.precision, han.recall, han.train_op], feed_dict)
            ...
            if epoch % FLAGS.validate_every == 0:
                eval_loss, eval_prec, eval_reca = do_eval(sess, han, batch_size)
...
def do_eval(sess, han, batch_size):
    ...
    while True:
        ...
        x, y = next_batch(test_input_data[start:end], batch_size, FLAGS.num_classes, FLAGS.sequence_length, han.w2vModel, FLAGS.embed_size)
        if x is None:
            break
        feed_dict = {han.input_x: x,
                     han.input_y: y,
        ...
        curr_eval_loss, logits, curr_eval_prec, curr_eval_reca = sess.run(
            [han.loss_val, han.logits, han.precision, han.recall], feed_dict)
