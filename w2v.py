...
def create_bucket():
    ...
    for result in results:
        i += 1
        data.append(tuple([int(result["pid"]), str(result["_id"]), len(result["readme_cleaned"].split(" "))]))
        if i % 1000 == 0:
            print("Processed %d" % i)
            cursor.executemany("INSERT INTO rdLength VALUES(%s, %s, %s) ON DUPLICATE KEY UPDATE rdLength = rdLength", data)
            mysql_db.commit()
            data = []
    if len(data) != 0:
        cursor.executemany("INSERT INTO rdLength VALUES(%s, %s, %s) ON DUPLICATE KEY UPDATE rdLength = rdLength", data)
